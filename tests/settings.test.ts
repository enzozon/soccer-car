...
test('preferências corrompidas retornam padrões seguros', () => {
  ...
  const value = validateSettings({ model: 'invasor', color: '<script>', quality: 42, volume: NaN, duration: -20 });
  assert.deepEqual(value, DEFAULT_SETTINGS);
});
test('preferências válidas preservadas e valores numéricos limitados', () => {
  const value = validateSettings({ model: 'rally', color: '#123ABC', quality: '2d', deadzone: 0, sensitivity: 8, volume: -1, duration: 60 });
